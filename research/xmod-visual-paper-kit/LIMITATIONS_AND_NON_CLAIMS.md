# X-MoD Limitations And Non-Claims

## Limitations

- The strongest alignment result is for the safety drive. Legality, comfort, and efficiency need their own event-alignment checks before making a full four-drive explainability claim.
- External baseline comparison is not yet strong enough for broad benchmark positioning.
- Closed-loop evidence is useful as stress-test support, but the primary discriminator in this kit is offline gate-event alignment and ablation evidence.
- The drive alignment status matrix is a boundary artifact: one measured drive, three pending drive-specific event protocols.
- The C2 partial-state expert result is mixed. It improves routing consistency in parts of the regime but should not be framed as universally better.
- This kit is a sanitized visual packet. It is not a full reproduction package.
- The included CSV is aggregate evidence only. It is not the raw dataset, training log, or CARLA workspace.

## Non-Claims

- No real-world autonomous-driving deployment readiness claim.
- No universal safety improvement claim.
- No external benchmark dominance claim.
- No claim that all four drives have equal validation evidence.
- No claim that the raw workspace is publication-ready as-is.

## Public Use Rule

Use the phrase "safety-gate alignment evidence under the included offline protocol" rather than broad safety or benchmark language.
